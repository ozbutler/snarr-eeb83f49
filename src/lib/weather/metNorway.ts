...
export const fetchMetNorwayForecast = createServerFn({ method: "POST" })
  .inputValidator((data: { lat: number; lon: number }) => data)
  .handler(async ({ data }): Promise<MetNorwayNormalized | null> => {
    try {
      ...
      const res = await fetch(url, {
        headers: {
          ...
          "User-Agent": "Snarr weather app forecast comparison / contact: support@snarr.app",
        },
      });
