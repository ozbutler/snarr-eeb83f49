...
import { ConfidenceBadge, VerifiedBadge, formatProviderList, getConfidenceExplanation } from "@/components/wb/Confidence";
...
function TodayPanel() {
  ...
  const { current, today, hourly, alerts, confidence, sources, updatedAt, sourceDetails } = forecast;
  ...
  const confidenceExplanation = getConfidenceExplanation(confidence, sourceDetails);
    ...
    <>
      ...
      >
        ...
        <div className="mt-3 flex items-center justify-between gap-2">
          <ConfidenceBadge level={confidence} sources={sources} details={sourceDetails} compact />
      ...
      >
        ...
        <div className="mt-2 space-y-1 text-xs text-muted-foreground">
          <p>{confidenceExplanation.providerText}</p>
          <p>{confidenceExplanation.agreement}</p>
          {confidenceExplanation.ranges ? <p>{confidenceExplanation.ranges}.</p> : null}
          {sources.length > 0 ? <p>Sources: {formatProviderList(sources)}.</p> : null}
        </div>
      </CollapsibleCard>
    ...
    <>
      <section className="rounded-3xl p-5 shadow-[var(--shadow-soft)]" style={{ background: "var(--gradient-sky)" }}>
        ...
        <div className="mt-2.5 flex items-center justify-between">
          <ConfidenceBadge level={forecast.confidence} sources={forecast.sources} details={forecast.sourceDetails} compact />
